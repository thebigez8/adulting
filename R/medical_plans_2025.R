fsahsahelp <- overlay(
  ...
  "$3200 in 2025 to an FSA (so that if you're on a family plan, you're stuck at $3200). On ",
  "the flip side, if you're on a single-coverage HDHP, you can contribute $4300 ",
  "in 2025 to an HSA, and $8550 if you have family coverage. Finally, note that ",
...
expenseshelp <- overlay(
  "expenses-help", button.class = "finance", as.html = TRUE,
  "Enter here the amount of medical expenses for each person, excluding copays (see the help button for copays ",
  "for more detail). Note that coverage for services, medications, etc. varies by plan type! In other words, ",
  "you might not enter the same number for all plans. Note also that traditional plans are symmetric ",
  "by person (that is: you can enter the people in any order). However, the HDHP is not symmetric; person 1 ",
  "is assumed to incur charges before person 2, etc. For a worst-case scenario, generally you would enter the lowest-",
  "costing person as person 1, and the most costly person as person 4."
)
...
html(
  ...
  HTMLhead(
    titl = "Medical Plan Comparison 2025",
    js = c("toggle", "overlay", "medical_plans_2020", "medical_plans_2025", "medical_plans_plot_2020"),
  ...
  body(
    ...
    h2("Medical Plan Comparison 2025"),
    div(
      class="row",
      div(
        ...
        div(
          ...
          div(class="left", overlay(
            ...
            " Setting this to 0 will recover the values presented in the benefits booklet, which ",
            "do not take taxes into account."
          ))
        ),
          ...
          tbody(
            ...
            tr(
              ...
              td(
                ...
                input(type="number", id="fsahsa1", value=3200, step=1, min=0, max=3200)
              ),
              td(
                label(`for`="fsahsa2", "FSA"),
                input(type="number", id="fsahsa2", value=3200, step=1, min=0, max=3200)
              ),
              td(
                label(`for`="fsahsa3", "HSA"),
                input(type="number", id="fsahsa3", value=8550, step=1, min=0, max=8550)
              )
            ),
            showifsmalltr("Maximum Rollover"),
            tr(
              th("Maximum Rollover", class="hideifsmall"),
              td(input(type="number", id="rollover1", value=640, step=1, min=0)),
              td(input(type="number", id="rollover2", value=640, step=1, min=0)),
              td(input(type="number", id="rollover3", value=8550, step=1, min=0, style = "display:none"))
            )
          ),
          tbody(
            id = "expenses",
            class="inputs-table bordered",
            showifsmalltr(html0(span("Person 1 ", class = "toggleable family"), "Medical Expenses", expenseshelp[[1]])),
            tr(
              th(
                html0(span("Person 1 ", class = "toggleable family"), "Medical Expenses", expenseshelp[[1]]),
                class="hideifsmall"
              ),
              td(input(type="number", id="cost11", value=500, step=1, min=0)),
              td(input(type="number", id="cost12", value=500, step=1, min=0)),
              td(input(type="number", id="cost13", value=500, step=1, min=0))
            ),
            ...
            tr(
              ...
              td(input(type="number", id="cost31", value=10000, step=1, min=0)),
              td(input(type="number", id="cost32", value=10000, step=1, min=0)),
              td(input(type="number", id="cost33", value=10000, step=1, min=0))
            ),
  ...
  write2file(file = "docs/finance/medical_plans_2025.html")
